...
return [
    ...
    'info' => [
         ...
         */
        'description' => '
Dokumentasi API untuk Radius Server. Gunakan API ini untuk mengelola NAS, User, dan Monitoring Koneksi. Dengan Authentikasi HMAC, setiap request ke API ini harus menyertakan API Key, Timestamp, dan Signature HMAC-SHA256 untuk memastikan keamanan dan integritas data.
        
        
        ### Autentikasi HMAC

Setiap request ke API ini wajib menyertakan Header berikut:
        * `X-API-Key`: API Key Anda
        * `X-Timestamp`: Format ISO8601 UTC (contoh: `2026-07-29T14:30:00Z`)
        * `X-Signature`: HMAC-SHA256 Signature

        #### Rumus Pembuatan Signature:
        ```text
        StringToSign = HTTP_METHOD + ":" + PATH + ":" + TIMESTAMP + ":" + MD5(REQUEST_BODY)
        Signature    = HMAC_SHA256(StringToSign, SECRET_KEY)
        ',
    ],

    'ui' => [
        'title' => "Dokumentas API Radius Server",
    ],
     ...
     */
    'servers' => [
        'Public API Server' => env('APP_URL', 'https://radius.gsmnet.co.id') . '/api',
    ],
